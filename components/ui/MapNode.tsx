...
export function MapNode(NodeProperties: NodeProps) {
  ...
  return (
      <Image src={BUBBLY} alt={"bubbly"} style={{filter: hexToCSSFilter(NodeProperties.colour).filter, position:"absolute", left: `${NodeProperties.x}px`, top: `${NodeProperties.y}px`, rotate: `${NodeProperties.heading ? NodeProperties.heading : 0}deg`}}></Image>
  );
} 