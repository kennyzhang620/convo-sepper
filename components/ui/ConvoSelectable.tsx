...
export function ConvoButton({className, href, width, paused, style, onClick }: ButtonProps) {
  ...
  return (
      <button className={className} onClick={onClick ? onClick : () => showOnClick(!showTrans)} style={style ? style : {padding:"1em"}}>
        <div style={{backgroundColor: paused ? "#FF9000": "#EEEDED", color: "#4D4D4D", display:"flex", filter: "drop-shadow(0 0 0.20rem black)"}}>
          ...
          <div style={{backgroundColor:"#FFE100", width: "40px"}}></div>
        </div>
        {showTrans ?
        <div className="text_content">
        <div id="ts${res[i].convo_id}" style={{backgroundColor: "#EEEDED", color: "#4D4D4D", filter: "drop-shadow(0 0 0.20rem black)", textAlign: "left", padding: "1em"}}>    
                  <h1>Transcript</h1>
                  <p style={{fontFamily: interF.className}}>bacd</p>
                      <h1>Tips</h1>
                      <p style={{fontFamily: interF.className}}>bacd</p>
                  </div>
              </div>
