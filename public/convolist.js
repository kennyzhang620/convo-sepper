...
function generateCell(res, max_size) {
	...
	for (var i = 0; i < res.length; i++) {
		if (inner != null && res != null && i < max_size) {
			...
			var html = `<div class="img_header" style='border: 1px solid black; border-radius: 10px; background-color: ${checkPaused(res[i].ids) ? "white" : document.getElementById(`point-${fformation[f]}`).style.backgroundColor}'>
